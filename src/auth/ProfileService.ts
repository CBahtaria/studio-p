  ...
  35 |     }
  36 |     if (!profile.email || !this.isValidEmail(profile.email)) {
  ...
  92 |     if (!ctx) {
  93 |       logger.error('ProfileService', 'Failed to get 2D canvas context');
  94 |       return ''; // Return empty string or a placeholder if context is unavailable
  95 |     }
